...
"@
		...
		ConvertTo-HTML -head $head -PostContent $observedTimelineHTML, $ApplicationDownloadStatisticsHTML, $JavascriptPostContent -PreContent $PreContent -Title $Title | Out-File "$ReportFullPath"
		$Success = $?
		...
		if (-not ($Success)) {
			...
			Write-Host "Get-IntuneManagementExtensionDiagnostics report HTML file created:`n`t$ReportFullPath`n" -ForegroundColor Green
		}
		...
		if(-not $DoNotOpenReportAutomatically) {
			...
			do {
				...
				try {
					$HTMLFile = Get-ChildItem "$ReportFullPath"
			...
			Write-Host "Opening created file:`n$ReportFullPath`n"
			try {
				Invoke-Item "$ReportFullPath"
			}
			catch {
				Write-Host "Error opening file automatically to browser. Open file manually:`n$ReportFullPath`n" -ForegroundColor Red
			}
		} else {
			...
			Write-Error "$($_.Exception.GetType().FullName)"
			Write-Error "$($_.Exception.Message)"
			Write-Error "Error creating HTML report: $ReportFullPath"
		}

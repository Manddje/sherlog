...
<style>%(css)s
  ...
  .report-wrap .note{color:var(--muted);margin:0}
  /* Native summary/timeline is the primary, website-styled view. */
  details.summary{background:var(--surface);border:1px solid var(--border);
    border-radius:12px;padding:.5rem 1.1rem;font-size:.92rem}
  details.summary>summary{cursor:pointer;font-weight:600;padding:.3rem 0;font-size:1rem}
  ...
  %(history)s
<script>
  // Native summary is primary: open it by default.
  document.querySelectorAll('main.report-wrap details.summary').forEach(d => d.open = true);
  // Lazy-load the original report iframe only when its panel is opened.
  document.querySelectorAll('details.report-orig').forEach(d => {
    function load() { const f = d.querySelector('iframe');
...
# to prefers-color-scheme. When framed it asks the parent for the real theme
# (sherlogThemeReq) and the parent replies via postMessage; the parent also
